...
begin
  media := (n1 + n2 + n3) / 3;
  // O Trunc corta a casa decimal do resultado de (média * 100) e depois volta com a casa decimal ao / a média por 100.
...
begin
  if (media >= 7) then
    begin
      lblResultadoFinal.Font.Color := clGreen;
      lblResultadoFinal.Caption := 'Aprovado!'
    end
  else if (media >= 5) and (media < 7) then
    begin
      lblResultadoFinal.Font.Color := clWebChocolate;
      lblResultadoFinal.Caption := 'Recuperação!';
    end
  else
    begin
      lblResultadoFinal.Font.Color := clRed;
